...
You are helping the user install **OpenCaddy** — a Telegram bot that remotely controls OpenCode coding agent sessions from a phone.
...
> 1. `/newbot` 전송
> 2. 봇 이름 입력 (예: My OpenCaddy)
> 3. 봇 유저네임 입력 (예: my_opencaddy_bot) — 반드시 `_bot`으로 끝나야 합니다
...
If the bot starts successfully (you'll see `OpenCaddy is running!` in the output), tell the user:
